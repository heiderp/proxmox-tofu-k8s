...
Un homelab enseña más por lo que rompe que por lo que instala. Tres del registro completo:

- **CoreDNS atascado en `ContainerCreating`:** `failed to find plugin "loopback" in path
  [/usr/lib/cni]`. El containerd de Debian busca los binarios CNI en `/usr/lib/cni`, que está
  vacío; Cilium los instala en `/opt/cni/bin`. Parece un fallo del CNI y es un default de la
  distribución.
- **Un patch de kubelet ignorado sin ningún error:** `patches:` va a nivel raíz de
  `InitConfiguration` en la API `v1beta4` de kubeadm. Dentro de `nodeRegistration` —donde iba en
  `v1beta3`— se ignora con un warning, y las reservas de memoria simplemente no se aplican.
- **`fio` reportando 1220 MiB/s y `fsync` en nanosegundos:** el test escribía en `/tmp`, que en
  Debian 13 es `tmpfs`. Medida inválida. Repetido sobre el disco real: p99 de `fdatasync` = 1,34 ms.

Registro completo, con causa y solución de cada una:
[tabla de incidencias](docs/BITACORA.md#registro-de-incidencias-transversales).

---

## Stack

| Capa | Herramienta | Versión | Nota |
|---|---|---|---|
| Hipervisor | Proxmox VE (Debian trixie) | 9 | ext4 + LVM-thin, no ZFS |
| Imagen base | Debian genericcloud + cloud-init | 13 | plantilla VMID 9000 |
| IaC | OpenTofu + provider `bpg/proxmox` | 1.12.6 / 0.111.1 | state cifrado con PBKDF2 |
| Config del SO | Ansible | 14.3.1 | prerequisitos y bootstrap de kubeadm |
| Kubernetes | kubeadm | 1.35.8 | 1 control plane + 2 workers |
| Runtime | containerd | 1.7.24 | cgroup driver `systemd` |
| CNI | Cilium en modo slim | 1.20.1 | sin Hubble; con NetworkPolicy |
...
Sin versión = aún no instalado. Lo fijado vive en
[`ansible/group_vars/all.yml`](ansible/group_vars/all.yml) y en los `values.yaml` de
[`gitops/infrastructure/`](gitops/infrastructure/), nunca en comandos sueltos.
